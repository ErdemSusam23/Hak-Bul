...
import { Scale, Send, Trash2, RotateCcw, Sparkles, LogOut, History, UserCircle2 } from 'lucide-react';
...
export default function SohbetSayfasi() {
    ...
    const gonder = useCallback(async () => {
        ...
        // Geçmişe ekle (kullanıcı giriş yapmışsa)
        if (kullanici?.email) {
    ...
    // Panelin dışına tıklanınca tarihi kapat
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (gecmisRef.current && !gecmisRef.current.contains(event.target)) {
                setGecmisAcik(false);
            }
        };
        if (gecmisAcik) document.addEventListener('mousedown', handleClickOutside);
    ...
    return (
        <div className="flex flex-col h-full">
            {/* ── NAVİGASYON ÇUBUĞU ── */}
            <header
                className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-white/6 relative z-50"
                style={{
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    ...
                    {kullanici ? (
                        <div className="flex items-center gap-1.5" ref={gecmisRef}>
                            <div className="relative">
                                <button
                                    onClick={() => setGecmisAcik(!gecmisAcik)}
                                    title="Arama Geçmişi"
                                    className={`p-2 rounded-xl transition-all ${gecmisAcik ? 'text-gold-400 bg-gold-400/10' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'}`}
                                >
                                    <History size={16} />
                                </button>
                                {gecmisAcik && (
                                    <GecmisPanel
                                        ...
                                        onSoruSec={(s) => { setGirdi(s); setGecmisAcik(false); inputRef.current?.focus(); }}
                                        onKapat={() => setGecmisAcik(false)}
                                    />
                                )}
                            </div>

                            <span className="text-slate-400 text-xs hidden sm:inline px-1 border-l border-white/10 pl-3">
                                {kullanici.email}
                            </span>
                            ...
                            <button
                                onClick={cikis}
                                title="Çıkış yap"
                                className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-400/5 rounded-xl transition-all ml-1"
                            >
                                <LogOut size={16} />
                            </button>
                        </div>
